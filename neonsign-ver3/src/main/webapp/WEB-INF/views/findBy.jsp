...
<div class="container">
   ...
   <h2 class="itjaMainTitle">Search 잇자!</h2>
   <!-- 태그명을 받아 현재 선택한 태그를 표시한다. -대협 -->
   <span id="getNowTagName"></span>
 <!-- Example row of columns -->
   <div class="row completeItjaList">
      <c:forEach items="${requestScope.list}" var="searchList">
         <!-- 카드 1개 -->
         <!-- name은 script.js에서 카드 현재 카드의 수를 구하기 위해 사용 -대협- -->
         <div class="card-box col-md-4" name="searchCardBox">
            <div class="card card-with-border" data-background="image"
               ...
               <div class="content">
                  ...
                  <c:set var="mainArticleContentTitle"
                     value="${searchList.mainArticleTitle}" />
                  <h5 class="title">
                  ...
                  <c:set var="mainArticleContentContent"
                     value="${searchList.mainArticleContent}" />
                  <p class="description">
               ...
               <a href="mypage.neon?memberEmail=${searchList.memberVO.memberEmail}" style="" tabindex="1" class="btn btn-lg btn-warning myNickDetail" role="button" 
                     data-toggle="popover" 
                     title="${searchList.memberVO.memberNickName}님, ${searchList.memberVO.rankingVO.memberGrade} PTS(${searchList.memberVO.memberPoint} / ${searchList.memberVO.rankingVO. maxPoint})" 
                     data-content="${searchList.memberVO.memberNickName}님 Click하여 페이지 보기" >
                  <span class="writersNickName">- ${searchList.memberVO.memberNickName} -</span>
               </a>
                 <input type="hidden" class="mainArticleTitleNO" value="${searchList.mainArticleNo}">
                  ...
                  <button class="btn btn-social btn-pinterest">
                   ...
                   <button class="btn btn-social btn-twitter itja">
                        ...
                        <c:forEach var="itjaList" items="${sessionScope.memberVO.itjaMemberList}">
                           <c:choose>
                              <c:when test="${itjaList.mainArticleNo==searchList.mainArticleNo}">
                        ...
                        <c:choose>
                           <c:when test="${count==true}">
                              <span class="itjaCount"><i class="fa fa-link"></i><br>${searchList.mainArticleTotalLike }it</span>
                           </c:when>
                           <c:otherwise>
                              <span class="itjaCount"><i class="fa fa-chain-broken"></i><br>${searchList.mainArticleTotalLike }it</span>
                           </c:otherwise>
                        </c:choose>
                            </button>
                            <form name="itJaInfo">
...
<body>
<h3>타이틀리스트</h3>
		<table border=2 class="table table-hover" id="findByTitleList"
			align="center">
			<thead align="center">
				<tr class="success">
				    <td>글번호</td>
			...
			<tbody align="center">
				<c:forEach items="${requestScope.list}" var="list">
					<tr>
						<td><a href="detailview.neon?no=+${list.mainArticleNo}"></a>${list.mainArticleNo}</td>
						<td>${list.mainArticleTitle}</td>
						<td>${list.mainArticleContent}</td>
						<td>${list.memberVO.memberNickName}</td>
						<td>${list.mainArticleHIt }</td>
